...
procedure TForm16.sortbylastname();
var  i, j, cnt :integer;
begin
   begin
    Cnt := PupilsListtable.RowCount;
      for j := 1 to Cnt - 1 do
        for i := j+1 to Cnt do
          begin
          if (PupilsListtable.Cells[2, i]='') or (PupilsListtable.Cells[2, j]='') then  continue;

            if AnsiLowerCase(PupilsListtable.Cells[2, i]) < AnsiLowerCase(PupilsListtable.Cells[2, j]) then
            with PupilsListtable do
            begin
            Rows[Cnt] := Rows[i];
            Rows[i] := Rows[j];
            Rows[j] := Rows[Cnt];
